...
async def async_main(board_format, model_name, client_type, hostname, batch_size, prompt_config):
    ...
    # dataset = load_from_disk("Lichess/chess-puzzles-3000-full-moves").select(range(100))
    dataset = load_from_disk("Lichess/one-move-chess-puzzles/test").select(range(10000))  
    dataset = dataset.map(lambda x: {"Puzzle_Moves_SAN": x['Moves']})
