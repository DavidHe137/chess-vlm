...
def process_puzzle_session(client, puzzle_session, model_name, board_format, logger):
    """Process a single puzzle session."""
    ...
    try:
        while puzzle_session.status == SessionStatus.ACTIVE:
            ...
            # Use Anthropic's messages API with prompt caching
...
@click.option('--batch_size', type=int, default=100,
              help="Batch size parameter (unused in sequential processing). Default is 100.")
...
def run_main(board_format, model_name, batch_size, prompt_config):
    ...
    results = []
    logger.info(f"Starting sequential evaluation of {total_puzzles} puzzles...")
    
    with tqdm(total=total_puzzles, desc="Processing puzzles") as pbar:
        for puzzle_session in puzzle_sessions:
            try:
                result = process_puzzle_session(client, puzzle_session, model_name, board_format, logger)
